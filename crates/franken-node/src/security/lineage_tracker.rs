...
mod tests {
    ...
    #[test]
    fn operator_secret_external_sink_acceptance_transcript_fails_closed_and_scopes_declassification()
     {
        ...
        let blocked_label_record = blocked_graph.labels.get(&blocked_label).unwrap();
        ...
        assert!(blocked_ledger.verify_graph_binding(
            &blocked_graph,
            "internal:.env",
            &blocked_label
        ));
        ...
        assert!(!blocked_label_record.description.contains("operator_secret"));
        assert!(
            !blocked_label_record
                .description
                .contains("operator-secret-salt")
        );
        ...
        assert!(allowed_ledger.verify_graph_binding(
            &allowed_graph,
            "internal:.env",
            &allowed_label
        ));
