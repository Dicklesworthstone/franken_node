...
mod tests {
    ...
    fn validation_optimization_produces_identical_field_paths() {
        ...
        assert!(result.is_err(), "Empty event_id should cause validation error");

        // Verify the error message contains the exact field path generated by optimized code
        let error_msg = format!("{}", result.unwrap_err());
        assert!(error_msg.contains("events[0].event_id"),
               "Error message should contain exact field path 'events[0].event_id', got: {}", error_msg);
        ...
        assert!(result2.is_err(), "Empty provenance_ref should cause validation error");

        let error_msg2 = format!("{}", result2.unwrap_err());
        assert!(error_msg2.contains("events[0].provenance_ref"),
               "Error message should contain exact field path 'events[0].provenance_ref', got: {}", error_msg2);
        ...
        assert!(result3.is_err(), "Empty event_id at index 1 should cause validation error");

        let error_msg3 = format!("{}", result3.unwrap_err());
        assert!(error_msg3.contains("events[1].event_id"),
               "Error message should contain exact field path 'events[1].event_id', got: {}", error_msg3);
        ...
        assert!(result4.is_err(), "Future timeline timestamp should cause validation error");

        let error_msg4 = format!("{}", result4.unwrap_err());
        assert!(error_msg4.contains("$.timeline[0].timestamp"),
               "Error message should contain exact field path '$.timeline[0].timestamp', got: {}", error_msg4);
    }
}
