//! bd-3a3q Guardrail Monitor Conformance Test Runner
//!
...
fn main() {
    ...
    let report = bd_3a3q_guardrail_monitor_conformance::run_bd_3a3q_conformance_tests();
    ...
    if compliance_score >= 0.95 {
        println!("✅ CONFORMANT - Meets bd-3a3q specification requirements");
    } else {
        println!("❌ NON-CONFORMANT - Does not meet bd-3a3q specification requirements");
    ...
    println!("  📄 JSON: bd_3a3q_conformance_report.json");

    // Markdown report for human consumption
    let markdown_report = report.to_markdown();
    std::fs::write("bd_3a3q_conformance_report.md", markdown_report)
        .expect("Failed to write Markdown report");
    println!("  📄 Markdown: bd_3a3q_conformance_report.md");
