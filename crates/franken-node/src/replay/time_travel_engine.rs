...
mod tests {
    ...
    fn negative_massive_trace_step_memory_pressure_testing() {
        ...
        // Start trace capture via the capture-phase builder
        let mut builder = TraceBuilder::new(massive_trace_id, "massive-workflow", env);
        ...
        for i in 0..massive_step_count {
            ...
            builder.record_step(
                ...
                output,
                side_effects,
                1000000_u64.saturating_add(u64::try_from(i).unwrap_or(u64::MAX)),
            );
        ...
        let capture_result = builder.build();
        ...
        match capture_result {
            Ok((trace, _)) => {
                ...
                engine
                    .register_trace(trace)
                    .expect("registering the bounded trace should succeed");
            }
            Err(_) => {
        ...
        let registered_traces = engine.trace_ids();
    ...
    fn negative_unicode_injection_in_trace_identifiers_and_workflow_names() {
        ...
        // Bind the long identifiers so the borrowed strings outlive the vec.
        let long_trace_id = "x".repeat(10000);
        let long_workflow_name = "y".repeat(10000);
        let malicious_identifiers = vec![
            ...
            (long_trace_id.as_str(), "workflow"),
            ("trace", long_workflow_name.as_str()),
        ];

        for (i, (trace_id, workflow_name)) in malicious_identifiers.iter().enumerate() {
            let env = demo_env();

            // Capture phase: building constructs the trace; identifier
            // validation is enforced at build()/registration time.
            let mut builder = TraceBuilder::new(trace_id, workflow_name, env);
            ...
            // Complete trace - malformed identifiers are rejected here.
            let complete_result = builder.build();
            ...
            match complete_result {
                Ok((trace, audit)) => {
                    ...
                    // Register so the engine listing can be exercised below.
                    // Duplicate identifiers are expected in the fixture set.
        ...
        let traces = engine.trace_ids();
        for trace_id in &traces {
            assert!(!trace_id.is_empty());
            let trace = engine
                .get_trace(trace_id)
                .expect("registered trace should be retrievable");
            assert!(!trace.workflow_name.is_empty());
        }
    }
    ...
    fn negative_extreme_timestamp_arithmetic_overflow_protection() {
        ...
        let mut builder = TraceBuilder::new(extreme_trace_id, "extreme-workflow", env);
        ...
        for (i, timestamp) in extreme_timestamps.iter().enumerate() {
            ...
            builder.record_step(input, output, side_effects, *timestamp);
        }

        // Complete trace with extreme timestamps
        let complete_result = builder.build();
        ...
        match complete_result {
            Ok((trace, audit)) => {
                ...
                for entry in &audit {
                    assert!(entry.timestamp_ns <= u64::MAX);
                }
                ...
                if !trace.steps.is_empty() {
                    engine
                        .register_trace(trace)
                        .expect("register extreme-timestamp trace");

                    let replay_result =
                        engine.replay_with_adapter(extreme_trace_id, |step, _env| {
    ...
    fn negative_malformed_side_effect_data_injection_attacks() {
        ...
        let mut builder = TraceBuilder::new(malicious_trace_id, "injection-test", demo_env());
    ...
    fn negative_divergence_detection_with_massive_output_differences() {
        ...
        let (trace, _) = builder.build().expect("complete capture");
        let trace_steps_len = trace.steps.len();
        engine
            .register_trace(trace)
            .expect("register divergence trace");

        // Replay with intentionally different outputs to trigger divergence detection
        let replay_result = engine.replay_with_adapter(divergent_trace_id, |step, _env| {
            ...
            Ok(result) => {
                ...
                if let ReplayVerdict::Diverged(divergence_count) = &result.verdict {
                    ...
                    for divergence in &result.divergences {
                        assert!(
                            divergence.step_seq
                                < u64::try_from(trace_steps_len).unwrap_or(u64::MAX)
                        );
                        assert!(!divergence.expected_digest.is_empty());
                        assert!(!divergence.actual_digest.is_empty());

                        // Expected and actual should be different
                        assert!(!constant_time::ct_eq_bytes(
                            divergence.expected_digest.as_bytes(),
                            divergence.actual_digest.as_bytes()
                        ));
                    }
                }
            }
    ...
    fn negative_environment_snapshot_corruption_and_tampering() {
        ...
        for (i, corrupted_env) in corrupted_environments.into_iter().enumerate() {
            ...
            // Capture phase: build the trace; corrupted environments are
            // rejected at build()/registration time.
            let mut builder = TraceBuilder::new(&trace_id, "env-test", corrupted_env);
            builder.record_step(
                b"test-input".to_vec(),
            ...
            match complete_result {
                Ok((trace, _)) => {
                    ...
                    assert!(trace.environment.clock_seed_ns <= u64::MAX);

                    // Test replay with potentially corrupted environment
                    engine
                        .register_trace(trace)
                        .expect("register env-corruption trace");

                    let replay_result = engine.replay_with_adapter(&trace_id, |step, _env| {
                        Ok((step.output.clone(), step.side_effects.clone()))
                    });
    ...
    fn negative_concurrent_trace_manipulation_and_race_conditions() {
        ...
        // Start trace capture via the capture-phase builder. While capture
        // is in progress the builder is `Some`; `complete_capture` consumes
        // it (registering the finalized trace), after which recording is no
        // longer possible.
        let mut builder = Some(TraceBuilder::new(
            concurrent_trace_id,
            "concurrent-workflow",
            demo_env(),
        ));
        ...
        for (operation, timestamp) in concurrent_operations {
            match operation {
                "complete_capture" => {
                    let completed = match builder.take() {
                        Some(b) => match b.build() {
                            Ok((trace, _)) => engine.register_trace(trace).is_ok(),
                            Err(_) => false,
                        },
                        None => false,
                    };
                    operation_results.push((operation, completed));
                }
                op if op.starts_with("record_step") => {
                    // Recording is only possible while the capture is in progress.
                    let recorded = match builder.as_mut() {
                        Some(b) => {
                            b.record_step(
                                format!("{}-input", operation).into_bytes(),
                                format!("{}-output", operation).into_bytes(),
                                vec![SideEffect::new(
                                    "concurrent",
                        ...
                        // Capture already completed -> cannot record.
                        None => false,
                    };
                    operation_results.push((operation, recorded));
                }
                _ => {}
            }
        }
        ...
        // Test conflicting trace operations: building a second capture for the
        // same id and attempting to register it must be handled gracefully
        // (the engine rejects the duplicate; no panic).
        let mut conflicting_builder =
            TraceBuilder::new(concurrent_trace_id, "conflicting-workflow", demo_env());
        conflicting_builder.record_step(
            b"conflict-input".to_vec(),
            b"conflict-output".to_vec(),
            vec![],
            3000,
        );
        let conflicting_results = vec![
            conflicting_builder
                .build()
                .and_then(|(trace, _)| engine.register_trace(trace)),
        ];
        ...
        let matching_traces: Vec<_> = traces
            .iter()
            .filter(|t| t.as_str().eq(concurrent_trace_id))
            .collect();
    ...
    fn metamorphic_serde_round_trip_workflow_trace_identity() {
        ...
        let steps = vec![
            TraceStep::new(
                0,
                b"input\x00\x01".to_vec(),
                b"output\xff\xfe".to_vec(),
                side_effects.clone(),
                1000,
            ),
            ...
            TraceStep::new(
                2,
                b"unicode_\xE6\xB5\x8B\xE8\xAF\x95".to_vec(),
                vec![],
                vec![SideEffect::new("unicode", "测试".bytes().collect())],
                3000,
            ),
        ];
        ...
        assert!(
            ct_eq(&original_trace.trace_digest, &deserialized.trace_digest),
        ...
        assert_eq!(
            original_trace.environment.schema_version,
            deserialized.environment.schema_version
        );
        assert_eq!(
            original_trace.environment.clock_seed_ns,
            deserialized.environment.clock_seed_ns
        );
        assert_eq!(
            original_trace.environment.platform,
            deserialized.environment.platform
        );
        assert_eq!(
            original_trace.environment.runtime_version,
            deserialized.environment.runtime_version
        );
        assert_eq!(
            original_trace.environment.env_vars.len(),
            deserialized.environment.env_vars.len()
        );
        for (key, orig_val) in &original_trace.environment.env_vars {
            let deser_val = deserialized
                .environment
                .env_vars
                .get(key)
            ...
            assert_eq!(
                orig_val, deser_val,
                "environment variable '{}' should be identical",
                key
            );
        }
    }
    ...
    fn metamorphic_serde_round_trip_trace_step_identity() {
        ...
        let deserialized: TraceStep =
            serde_json::from_str(&serialized).expect("deserialize step should succeed");
    ...
    fn metamorphic_serde_round_trip_environment_snapshot_identity() {
        ...
        let deserialized: EnvironmentSnapshot =
            serde_json::from_str(&serialized).expect("deserialize env should succeed");
        ...
        for (key, value) in &env.env_vars {
            assert_eq!(
                deserialized.env_vars.get(key),
                Some(value),
                "env var '{}' should have identical value after round-trip",
                key
            );
        }
    }
    ...
    #[test]
    fn metamorphic_digest_side_effects_order_dependence_generated_cases() {
        ...
        runner
            .run(&strategy, |pairs| {
                let side_effects: Vec<SideEffect> = pairs
                    .into_iter()
                    .enumerate()
                    .map(|(idx, (kind_bytes, mut payload))| {
                        let kind_suffix = if kind_bytes.is_empty() {
                            "empty".to_string()
                        } else {
                            hex::encode(kind_bytes)
                        };
                ...
                prop_assert_ne!(side_effects.clone(), rotated.clone());

                let base_step =
                    TraceStep::new(0, b"input".to_vec(), b"output".to_vec(), side_effects, 1000);
                let rotated_step =
                    TraceStep::new(0, b"input".to_vec(), b"output".to_vec(), rotated, 1000);

                prop_assert_ne!(
                    base_step.side_effects_digest(),
                    rotated_step.side_effects_digest(),
                    "side-effects digest must remain order-dependent across generated permutations"
                );
                Ok(())
            })
            .expect(
                "generated side-effect permutations should preserve documented ordering semantics",
            );
    }
    ...
    fn metamorphic_digest_environment_vars_insertion_order_independence() {
        ...
        eprintln!(
            "✅ Environment variables correctly order-independent (BTreeMap canonical ordering)"
        );
    }

    #[test]
    fn metamorphic_digest_environment_vars_insertion_order_independence_generated_cases() {
        let mut runner = TestRunner::default();
        let strategy = prop::collection::vec(
            (
                prop::collection::vec(any::<u8>(), 0..=16),
        ...
        runner
            .run(&strategy, |pairs| {
                let entries: Vec<(String, String)> = pairs
                    .into_iter()
                    .enumerate()
                    .map(|(idx, (key_bytes, value_chars))| {
                        let key_suffix = if key_bytes.is_empty() {
                            "EMPTY".to_string()
                        } else {
                            hex::encode(key_bytes)
                        };
                        let value: String = value_chars.into_iter().collect();
                        (format!("KEY_{idx}_{key_suffix}"), value)
                    })
                    .collect();
                ...
                prop_assert_eq!(
                    serde_json::to_string(&env1).expect("serialize generated env1"),
                    serde_json::to_string(&env2).expect("serialize generated env2"),
                    "BTreeMap-backed environment snapshots must ignore insertion order for generated env vars"
                );
                Ok(())
            })
            .expect("generated environment permutations should preserve canonical ordering");
    }
    ...
    fn negative_replay_function_exception_and_panic_handling() {
        ...
        engine
            .register_trace(trace)
            .expect("register exception trace");
        ...
        for (fail_step, error_type) in failure_modes {
            let replay_result = engine.replay_with_adapter(exception_trace_id, |step, _env| {
                if step.seq.eq(&fail_step) {
                    Err(TimeTravelError::ReplayFailed {
                        trace_id: exception_trace_id.to_string(),
                        reason: format!("Simulated {} at step {}", error_type, fail_step),
                    })
                } else {
                    Ok((step.output.clone(), step.side_effects.clone()))
                }
        ...
        let corrupted_replay_result =
            engine.replay_with_adapter(exception_trace_id, |_step, _env| {
    ...
    fn hardening_audit_log_prevents_unbounded_growth() {
        ...
        let mut builder = TraceBuilder::new(&trace_id, "audit-test-workflow", demo_env());
        ...
        for i in 0..10000u64 {
            ...
            builder.record_step(input, output, effects, i);
        }

        // Complete capture and collect the accumulated audit log
        let (_trace, audit_logs) = builder.build().expect("capture complete should succeed");
    ...
    fn hardening_side_effects_bounded_accumulation() {
        ...
        let mut builder = TraceBuilder::new(&trace_id, "effects-test", demo_env());
        ...
        builder.record_step(input, output, massive_effects, 1000);

        // Should complete without panic - effects are bounded by the builder.
        match builder.build() {
            Ok((trace, _)) => {
    ...
    fn hardening_trace_digest_comparison_timing_safe() {
        ...
        // Create a valid trace via the capture-phase builder
        let mut builder = TraceBuilder::new(&trace_id, "timing-workflow", demo_env());
        builder.record_step(b"input".to_vec(), b"output".to_vec(), vec![], 1000);
        let (trace, _) = builder.build().expect("capture complete should succeed");

        // Get the trace's digest
    ...
    fn hardening_step_sequence_overflow_protection() {
        ...
        let mut builder = TraceBuilder::new(&trace_id, "sequence-test", demo_env());
        ...
        for seq in boundary_sequences {
            ...
            // Record step with explicit boundary timestamp; the builder assigns
            // the actual sequence number via saturating arithmetic.
            builder.record_step(input, output, effects, seq);
        }

        // Should handle without panic - verify recorded sequences are bounded
        if let Ok((trace, _)) = builder.build() {
    ...
    fn hardening_environment_data_length_validation() {
        ...
        // Try to capture with a malicious environment; environment validation
        // occurs at build()/registration time.
        let mut builder = TraceBuilder::new(&trace_id, "env-test", malicious_env_snapshot);
        builder.record_step(b"input".to_vec(), b"output".to_vec(), vec![], 1000);
    ...
    fn negative_domain_separator_hash_collision_resistance_verification() {
        ...
        assert_ne!(
            b"replay_step_output_v1:".as_slice(),
            b"replay_step_effects_v2:".as_slice(),
            "Step domain prefixes must be distinct"
        );
        assert_ne!(
            b"replay_trace_digest_v1:".as_slice(),
            b"replay_step_output_v1:".as_slice(),
    ...
    fn replay_fixture_identity_idempotency_metamorphic_invariant() {
        ...
        assert_eq!(
            ...
            "replay verdicts must match"
        );
        assert_eq!(
            result_a.steps_replayed, result_b.steps_replayed,
            "steps replayed count must match"
        );
        assert_eq!(
            result_a.divergences, result_b.divergences,
            "divergences must be identical"
        );

        // METAMORPHIC ASSERTION 2: Engine states must be byte-identical (canonical serialization)
        let engine_a_bytes =
            serde_json::to_vec(&engine_a.audit_log()).expect("engine_a audit log should serialize");
        let engine_b_bytes =
            serde_json::to_vec(&engine_b.audit_log()).expect("engine_b audit log should serialize");
        ...
        assert_eq!(
            engine_a.trace_count(),
            engine_b.trace_count(),
            "trace counts must match"
        );
        assert_eq!(
            engine_a.trace_ids(),
            engine_b.trace_ids(),
            "trace IDs must be identical"
        );

        let trace_a = engine_a
            .get_trace("idempotency-test")
            .expect("trace should exist in engine_a");
        let trace_b = engine_b
            .get_trace("idempotency-test")
            .expect("trace should exist in engine_b");
        ...
        let result_a_bytes = serde_json::to_vec(&result_a).expect("result_a should serialize");
        let result_b_bytes = serde_json::to_vec(&result_b).expect("result_b should serialize");
    ...
    fn replay_idempotency_same_engine_multiple_runs() {
        ...
        let mut engine = ReplayEngine::new();
        engine.register_trace(trace).expect("register trace");
        ...
        assert_eq!(
            result_1.verdict, result_2.verdict,
            "first and second replay verdicts must match"
        );
        assert_eq!(
            result_2.verdict, result_3.verdict,
            "second and third replay verdicts must match"
        );

        assert_eq!(
            result_1.steps_replayed, result_2.steps_replayed,
            "steps replayed must be consistent"
        );
        assert_eq!(
            result_2.steps_replayed, result_3.steps_replayed,
            "steps replayed must be consistent"
        );

        assert_eq!(
            result_1.divergences, result_2.divergences,
            "divergences must be identical across runs"
        );
        assert_eq!(
            result_2.divergences, result_3.divergences,
            "divergences must be identical across runs"
        );
    ...
    fn canonicalize_environment(env: &TestEnvironment) -> TestEnvironment {
        ...
        canonical_env.env_vars = canonical_env
            .env_vars
            .into_iter()
    ...
    #[test]
    fn environment_snapshot_canonicalization_commutes_for_generated_canonical_inputs() {
        let mut runner = TestRunner::default();
        let strategy = (
            any::<u64>(),
            prop::collection::vec(
            ...
            .run(
                ...
                |(clock_seed_ns, env_pairs, platform_bytes, version_parts)| {
                    let env_vars = env_pairs
                        .into_iter()
                        .enumerate()
                        .map(|(idx, (key_bytes, value_bytes))| {
                            let key_suffix = if key_bytes.is_empty() {
                                "EMPTY".to_string()
                            } else {
                                hex::encode(key_bytes)
                            };
                            (format!("KEY_{idx}_{key_suffix}"), hex::encode(value_bytes))
                        })
                        .collect();
                    ...
                    let test_env = TestEnvironment {
                        clock_seed_ns,
                        env_vars,
                        platform: format!("linux-{}", hex::encode(platform_bytes)),
                    ...
                    prop_assert_eq!(canonical_bytes_path1, canonical_bytes_path2);
                    prop_assert_eq!(snapshot_first, snapshot_second);
                    Ok(())
                },
            )
            .expect(
    ...
    fn clock_drift_wraparound_boundary_calculation() {
        ...
        let wrapped_timestamp: u64 = 50; // Wrapped around to small value
        ...
        let (trace, _) = builder.build().expect("build wraparound trace");
        ...
        let wraparound_replay_fn = |step: &TraceStep, _env: &EnvironmentSnapshot| {
            ...
            Ok((step.output.clone(), effects))
        };

        let result = engine
            .replay_with_adapter("wraparound-test", wraparound_replay_fn)
            .unwrap();
