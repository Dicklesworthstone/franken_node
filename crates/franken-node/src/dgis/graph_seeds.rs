...
pub enum SeedError {
    ...
    InvalidObservation { index: usize, source: IngestError },
    /// `ingest` or `finalize_window` rejected an observation during
    /// [`build_windowed_graph_from_seed`].
    IngestRejected { index: usize, source: IngestError },
}
...
impl GraphSeed {
    ...
    pub fn validate(&self) -> Result<(), SeedError> {
        ...
        for (i, obs) in self.observations.iter().enumerate() {
            obs.validate().map_err(|e| SeedError::InvalidObservation {
                index: i,
                source: e,
            })?;
        }
...
pub fn realistic_npm_topology() -> GraphSeed {
    let observations = vec![
        ...
        obs(
            ...
            "1.2.0",
            &["alice", "bob"],
            &[("lodash-lite", "^4.0.0"), ("json-utils", "^1.5.0")],
            Some("ac11ec01"),
        ),
        obs(
            90_000,
            ...
            "2.0.1",
            &["alice", "carol"],
            &[("acme-core", "^1.2.0"), ("http-client", "^2.3.0")],
            Some("ac11ec02"),
        ),
        obs(
            100_000,
            ...
            "0.9.0",
            &["bob"],
            &[("acme-api", "^2.0.0"), ("yargs-mini", "^1.0.0")],
            Some("ac11ec03"),
        ),
        obs(
            110_000,
        ...
        obs(
            ...
            "1.0.5",
            &["dave"],
            &[("globex-db", "^3.1.0"), ("validator-x", "^0.8.0")],
            None,
        ),
        ...
        obs(
            ...
            "2.0.2",
            &["alice", "carol"],
            &[("acme-core", "^1.2.1"), ("http-client", "^2.3.0")],
            Some("ac11ec02"),
        ),
        obs(
            1_200_000,
            ...
            "0.9.1",
            &["bob"],
            &[("acme-api", "^2.0.2"), ("yargs-mini", "^1.0.0")],
            Some("ac11ec03"),
        ),
        obs(
            1_300_000,
        ...
        obs(
            ...
            "2.0.3",
            &["alice", "carol"],
            &[("acme-core", "^1.2.2"), ("http-client", "^2.3.0")],
            Some("ac11ec02"),
        ),
        obs(
            2_450_000,
        ...
        obs(
            ...
            "0.9.2",
            &["bob"],
            &[("acme-api", "^2.0.3"), ("yargs-mini", "^1.0.0")],
            Some("ac11ec03"),
        ),
        obs(
            2_550_000,
...
mod tests {
    ...
    fn realistic_npm_topology_loads_without_error() {
        ...
        seed.validate()
            .expect("realistic_npm_topology must validate");
    ...
    fn realistic_npm_topology_satisfies_expected_invariants() {
        ...
        assert_eq!(
            graph.total_observations as usize, invariants.expected_unique_observations,
