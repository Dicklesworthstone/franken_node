...
mod tests {
            ...
            if let Some(proposed) = proposed_level {
                if (current_level as u8) > (proposed as u8) {
                    assert!(matches!(
                        ...
                        GuardrailVerdict::Block { ref budget_id, .. }
                            if budget_id.as_str() == "hardening_regression"
                    ), "Downgrade from {:?} to {:?} should be blocked", current_level, proposed);
                }
            }
        }
    }
    ...
    fn policy_conformal_risk_statistical_manipulation_confidence_bypass_attack() {
        ...
        for (sample_count, nonconforming_count) in manipulation_attacks {
            ...
            if sample_count >= 16 {
                let empirical_rate = nonconforming_count.min(sample_count) as f64 / sample_count as f64;

                if empirical_rate > 0.15 { // Well above acceptable thresholds
                    assert!(matches!(
                        verdict,
                        GuardrailVerdict::Block { ref budget_id, .. }
                            if budget_id.as_str() == "conformal_risk"
                    ), "High error rate {:.3} should be blocked", empirical_rate);
                }

                // Ensure no integer overflow occurred
                assert!(empirical_rate.is_finite() && empirical_rate >= 0.0 && empirical_rate <= 1.0,
                    "Empirical rate must be finite and in [0,1]: {}", empirical_rate);
            }
        }
    }
    ...
    fn policy_concurrent_guardrail_evaluation_state_corruption_race_attack() {
        ...
        assert!(matches!(
            final_verdict,
            GuardrailVerdict::Allow | GuardrailVerdict::Warn { .. } | GuardrailVerdict::Block { .. }
        ));
    }
    ...
    fn policy_guardrail_complexity_memory_exhaustion_dos_attack() {
        ...
        for i in 0..1000 { // Limit to prevent actual DoS in test
        ...
        for (memory_used, memory_budget, durability) in stress_states {
            ...
            assert!(elapsed.as_millis() < 1000,
                "Guardrail evaluation took {}ms, should be <1000ms", elapsed.as_millis());
    ...
    fn policy_floating_point_precision_threshold_bypass_attack() {
        ...
        for (mean_utilization, peak_utilization) in precision_attacks {
            ...
            assert!(mean_utilization.is_finite() && peak_utilization.is_finite(),
                "Input values must be finite: mean={}, peak={}", mean_utilization, peak_utilization);

            // Results should be consistent regardless of precision nuances
            match verdict {
                GuardrailVerdict::Allow => {
                    // If allowed, both values should be clearly below thresholds
                    assert!(mean_utilization < 0.85 && peak_utilization < 0.95,
                        "Precision attack bypassed thresholds: mean={}, peak={}",
                        mean_utilization, peak_utilization);
                },
                GuardrailVerdict::Warn { .. } => {
    ...
    fn policy_malformed_telemetry_injection_validation_bypass_attack() {
        ...
        let malformed_attacks = [
            ...
            (f64::MIN_POSITIVE, f64::MIN_POSITIVE * 0.5, f64::MIN_POSITIVE * 2.0),
        ...
        for (mean_util, variance_util, peak_util) in malformed_attacks {
            ...
            match verdict {
                GuardrailVerdict::Allow => {
                    ...
                    if !mean_util.is_finite() || !variance_util.is_finite() || !peak_util.is_finite() {
                        panic!("Malformed telemetry should not result in Allow verdict: mean={}, var={}, peak={}",
        ...
        for (sample_count, nonconforming_count) in overflow_scenarios {
            ...
            match verdict {
                GuardrailVerdict::Allow => {
                    ...
                    let rate = nonconforming_count.min(sample_count) as f64 / sample_count.max(1) as f64;
                    assert!(rate.is_finite() && rate >= 0.0 && rate <= 1.0,
                        "Invalid rate calculation: {}", rate);
                },
                GuardrailVerdict::Warn { .. } | GuardrailVerdict::Block { .. } => {
    ...
    fn negative_policy_unicode_injection_comprehensive() {
        use super::guardrail_monitor::{GuardrailMonitor, MemoryBudgetGuardrail, SystemState, GuardrailVerdict};
        use super::hardening_state_machine::HardeningLevel;
        ...
        let unicode_attack_vectors = vec![
            ...
            ("bidi_nested", "policy\u{202E}level1\u{202E}level2\u{202D}evil\u{202D}"),

            // Zero-width character pollution
            ("zws_pollution", "guard\u{200B}evil\u{200C}rail\u{200D}"),
            ("zwj_sequence", "policy\u{200D}\u{1F469}\u{200D}\u{1F4BB}trusted"),
            ...
            ("combining_stack", "guard\u{0300}\u{0301}\u{0302}\u{0303}rail"),
            ...
            let injection_result = std::panic::catch_unwind(|| {
                ...
                match verdict {
                    ...
                    GuardrailVerdict::Warn { reason } => {
                        assert!(!reason.is_empty(), "Warn reason should be present: {}", attack_name);
                    }
                    GuardrailVerdict::Block { reason } => {
                        assert!(!reason.is_empty(), "Block reason should be present: {}", attack_name);
                    }
                }

                // Test guardrail debug formatting with Unicode
                let debug_output = format!("{:?}", guardrail);
                assert!(!debug_output.contains("\u{202E}"), "Debug should not contain BiDi overrides: {}", attack_name);
                assert!(!debug_output.contains("\x1b["), "Debug should escape ANSI sequences: {}", attack_name);

                // Test system state serialization safety
                let state_debug = format!("{:?}", state);
                assert!(!state_debug.contains("%s"), "State debug should not contain format specifiers: {}", attack_name);
                assert!(!state_debug.contains("\x00"), "State debug should escape null bytes: {}", attack_name);

                // Test hardening level enum with Unicode context
                let hardening_debug = format!("{:?}", HardeningLevel::Enhanced);
                assert!(!hardening_debug.is_empty(), "Hardening level should have debug representation: {}", attack_name);
    ...
    fn negative_policy_memory_exhaustion_stress() {
        use super::guardrail_monitor::{
            GuardrailMonitorSet, MemoryBudgetGuardrail, DurabilityLossGuardrail,
            EvidenceEmissionGuardrail, HardeningRegressionGuardrail,
            ConformalRiskGuardrail, MemoryTailRiskGuardrail, SystemState
        };
        use super::hardening_state_machine::HardeningLevel;
        ...
        let memory_stress_result = std::panic::catch_unwind(|| {
            ...
            for (memory_used, memory_budget, test_name) in extreme_memory_cases {
                ...
                state.reliability_telemetry = Some(super::guardrail_monitor::ReliabilityTelemetry {
                ...
                match conformal_verdict {
                    GuardrailVerdict::Allow => {
                        ...
                        if let Some(telemetry) = &state.reliability_telemetry {
                            let rate = telemetry.nonconforming_count as f64 / telemetry.sample_count.max(1) as f64;
                            assert!(rate.is_finite(), "Rate calculation should be finite: {}", test_name);
                            assert!(rate >= 0.0 && rate <= 1.0, "Rate should be in valid range: {}", test_name);
                        }
                    }
                    GuardrailVerdict::Warn { .. } | GuardrailVerdict::Block { .. } => {}
                }
            ...
            for i in 0..1000 {
                ...
                if i % 100 == 0 {
                    let test_state = policy_state();
                    let check_result = std::panic::catch_unwind(|| monitor_set.check_all(&test_state));
                    assert!(check_result.is_ok(), "Monitor set should handle {} guardrails", i + 1);
                }
            }
            ...
            assert!(final_result.len() <= 1000, "Should not exceed expected guardrail count");
    ...
    fn negative_policy_json_integrity_validation() {
        use super::guardrail_monitor::{SystemState, GuardrailVerdict};
        use super::hardening_state_machine::HardeningLevel;
        ...
        for (malicious_pattern, attack_name) in json_corruption_patterns {
            let json_integrity_result = std::panic::catch_unwind(|| {
                ...
                assert!(!state_debug.contains("%s"), "State debug should not contain format specifiers: {}", attack_name);
                assert!(!state_debug.contains("\x1b["), "State debug should escape ANSI sequences: {}", attack_name);
                assert!(!state_debug.contains("\r\n{"), "State debug should not allow JSON injection: {}", attack_name);
                ...
                for hardening_level in hardening_variants {
                    ...
                    assert!(!hardening_debug.contains("%s"), "Hardening debug should be safe: {}", attack_name);
                    assert!(!hardening_debug.contains("\x1b["), "Hardening debug should escape ANSI: {}", attack_name);
                }

                // Test GuardrailVerdict formatting integrity
                let verdict_cases = vec![
                    GuardrailVerdict::Allow,
                    GuardrailVerdict::Warn { reason: malicious_pattern.to_string() },
                    GuardrailVerdict::Block { reason: malicious_pattern.to_string() },
                ];

                for verdict in verdict_cases {
                    let verdict_debug = format!("{:?}", verdict);
                    assert!(!verdict_debug.contains("\x1b["), "Verdict debug should escape ANSI: {}", attack_name);

                    match verdict {
                        GuardrailVerdict::Warn { reason } | GuardrailVerdict::Block { reason } => {
                            // Verify reason is preserved but safely formatted
                            assert!(verdict_debug.contains(&reason) || verdict_debug.len() > 10,
                                   "Verdict should preserve or escape reason: {}", attack_name);
                        }
                        GuardrailVerdict::Allow => {}
                    }
                }
                ...
                let complex_debug = format!("{:?}", (test_state, HardeningLevel::Strict, GuardrailVerdict::Allow));
                assert!(complex_debug.len() > 50, "Complex debug should have substantial content: {}", attack_name);
                assert!(!complex_debug.contains("\\u0000"), "Complex debug should escape null properly: {}", attack_name);
    ...
    fn negative_policy_arithmetic_overflow_protection() {
        use super::guardrail_monitor::{
            MemoryBudgetGuardrail, ConformalRiskGuardrail, SystemState,
            ReliabilityTelemetry, GuardrailVerdict
        };
        use super::hardening_state_machine::HardeningLevel;
        ...
        let overflow_protection_result = std::panic::catch_unwind(|| {
            ...
            for (memory_used, memory_budget, test_name) in memory_overflow_cases {
                ...
                if memory_budget > 0 {
                    ...
                    assert!(usage_ratio.is_finite() || usage_ratio.is_infinite(),
                           "Usage ratio should be mathematically valid: {}", test_name);
                }
            }
            ...
            for (sample_count, nonconforming_count, test_name) in conformal_overflow_cases {
                ...
                match verdict {
                    GuardrailVerdict::Allow => {
                        ...
                        if sample_count > 0 {
                            let rate = (nonconforming_count.min(sample_count) as f64) / (sample_count as f64);
            ...
            for epoch_id in epoch_overflow_cases {
                ...
                assert!(debug_output.contains(&epoch_id.to_string()) || debug_output.len() > 50,
                       "Debug should handle epoch ID: {}", epoch_id);
            }
            ...
            for (warn_threshold, block_threshold, test_name) in threshold_cases {
                ...
                assert!(!guardrail_debug.is_empty(), "Guardrail debug should have content: {}", test_name);
            }
    ...
    fn negative_policy_concurrent_access_safety() {
        ...
        let concurrent_safety_result = std::panic::catch_unwind(|| {
            ...
            for thread_id in 0..3 { // Limit threads for test stability
                let monitor_clone = Arc::clone(&monitor_set);
                                ...
                                match verdict {
                                    ...
                                    GuardrailVerdict::Warn { reason } => {
                                        assert!(!reason.is_empty(),
                                               "Warn reason should not be empty in thread {}", thread_id);
                                    },
                                    GuardrailVerdict::Block { reason } => {
                                        assert!(!reason.is_empty(),
                                               "Block reason should not be empty in thread {}", thread_id);
                                    },
                                }
                            }
                        }
            ...
            if let Ok(final_monitor) = monitor_set.lock() {
                ...
                for verdict in final_verdicts {
                    ...
                    let verdict_debug = format!("{:?}", verdict);
                    assert!(!verdict_debug.is_empty(), "Verdict should have debug representation");
                }
            }

            // Test concurrent state updates with safety
            let shared_state = Arc::new(Mutex::new(policy_state()));
            let update_handles: Vec<_> = (0..2).map(|thread_id| {
                ...
                thread::spawn(move || {
                    for i in 0..10 {
                        if let Ok(mut state) = state_clone.lock() {
                            ...
                            state.memory_used_bytes = state.memory_used_bytes.saturating_add(thread_id + i);
                            state.epoch_id = state.epoch_id.saturating_add(1);

                            // Test that state remains valid after updates
                            assert!(state.memory_used_bytes < u64::MAX, "Memory should not overflow");
                            assert!(state.durability_level >= 0.0 && state.durability_level <= 1.0,
            ...
            for handle in update_handles {
                handle.join().expect("State update thread should complete without panic");
            }

            // Verify final shared state is consistent
            if let Ok(final_shared_state) = shared_state.lock() {
                assert!(final_shared_state.memory_used_bytes < u64::MAX, "Final memory should be valid");
                assert!(final_shared_state.epoch_id > 0, "Final epoch should be positive");
            }

            Ok(())
        });

        assert!(concurrent_safety_result.is_ok(), "Concurrent access safety test should not panic");
    }
    ...
    fn negative_policy_display_injection_safety() {
        use super::guardrail_monitor::{
            MemoryBudgetGuardrail, SystemState, GuardrailVerdict
        };
        use super::hardening_state_machine::HardeningLevel;
        ...
        let display_injection_vectors = vec![
            ...
            ("combining_overflow", "reason\u{0300}\u{0301}\u{0302}\u{0303}"),
        ...
        for (attack_name, malicious_content) in display_injection_vectors {
            let display_safety_result = std::panic::catch_unwind(|| {
                ...
                let verdicts = vec![
                    GuardrailVerdict::Allow,
                    GuardrailVerdict::Warn { reason: malicious_content.to_string() },
                    GuardrailVerdict::Block { reason: malicious_content.to_string() },
                ];

                for verdict in verdicts {
                    // Test debug display formatting safety
                    let verdict_display = format!("{:?}", verdict);
                    assert!(!verdict_display.contains("%s"), "Verdict display should not contain format specifiers: {}", attack_name);
                    assert!(!verdict_display.contains("\x1b["), "Verdict display should escape ANSI sequences: {}", attack_name);
                    assert!(!verdict_display.contains("\r\n[FAKE]"), "Verdict display should not allow log injection: {}", attack_name);

                    match verdict {
                        GuardrailVerdict::Warn { reason } | GuardrailVerdict::Block { reason } => {
                            // Verify reason is preserved but safely displayed
                            assert_eq!(reason, malicious_content, "Reason should be preserved: {}", attack_name);
                        }
                        GuardrailVerdict::Allow => {}
                    }
                }
                ...
                assert!(!state_display.contains("%s"), "State display should not contain format specifiers: {}", attack_name);
                assert!(!state_display.contains("\x1b["), "State display should escape ANSI sequences: {}", attack_name);
                ...
                for level in hardening_levels {
                    ...
                    assert!(!level_display.contains("%s"), "Hardening level display should be safe: {}", attack_name);
                    assert!(!level_display.contains("\x1b["), "Hardening level display should escape ANSI: {}", attack_name);
                }

                // Test MemoryBudgetGuardrail display safety
                let guardrail = MemoryBudgetGuardrail::new(0.8, 0.6);
                let guardrail_display = format!("{:?}", guardrail);
                assert!(!guardrail_display.contains("%s"), "Guardrail display should be safe: {}", attack_name);
                assert!(!guardrail_display.contains("\x1b["), "Guardrail display should escape ANSI: {}", attack_name);
    ...
    fn negative_policy_boundary_stress_comprehensive() {
        use super::guardrail_monitor::{
            MemoryBudgetGuardrail, DurabilityLossGuardrail, ConformalRiskGuardrail,
            SystemState, GuardrailVerdict, ReliabilityTelemetry
        };
        use super::hardening_state_machine::HardeningLevel;
        ...
        let boundary_stress_result = std::panic::catch_unwind(|| {
            ...
            let threshold_boundaries = vec![
                ...
                (1.0 - f64::EPSILON, 1.0 - f64::EPSILON, "near_one_thresholds"),
            ];
            ...
            for (durability_level, test_name) in durability_boundaries {
                ...
                let state_debug = format!("{:?}", state);
                assert!(state_debug.contains(&durability_level.to_string()) || state_debug.len() > 50,
                       "State should represent durability level: {}", test_name);
            }
            ...
            for epoch_id in epoch_boundaries {
                ...
                let state_debug = format!("{:?}", state);
                assert!(!state_debug.is_empty(), "State should have debug representation for epoch: {}", epoch_id);
            ...
            for hardening_level in hardening_levels {
                ...
                let state_debug = format!("{:?}", state);
                assert!(!state_debug.is_empty(), "State should represent hardening level: {:?}", hardening_level);
                ...
                for evidence_active in [true, false] {
                    ...
                    assert!(evidence_debug.contains(&evidence_active.to_string()) || evidence_debug.len() > 50,
                           "State should represent evidence emission: {}", evidence_active);
                }
            }
