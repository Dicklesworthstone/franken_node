...
mod tests {
    ...
    fn audit_log_respects_bounded_capacity() {
        ...
        // Verify audit log size is capped at MAX_VERIFIER_AUDIT_LOG_ENTRIES
        {
            ...
            // `entry_id` is now auto-generated by `next_audit_entry_id`; the eviction order is
            // asserted via the caller-controlled `action` field (the closest current equivalent).
            let first_preserved_idx = test_capacity - MAX_VERIFIER_AUDIT_LOG_ENTRIES;
            assert_eq!(
                state.audit_log[0].action,
                format!("action_{}", first_preserved_idx),
                "oldest entry should be from index {}",
                first_preserved_idx
            );
            assert_eq!(
                state.audit_log.last().unwrap().action,
                format!("action_{}", test_capacity - 1),
