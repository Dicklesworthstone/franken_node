...
fn compute_compilation_digest(normalised_text: &str, evidence_links: &[EvidenceLink]) -> String {
    ...
    hasher.update(
        u64::try_from(normalised_text.len())
            .unwrap_or(u64::MAX)
            .to_le_bytes(),
    );
    hasher.update(normalised_text.as_bytes());
    hasher.update(
        u64::try_from(evidence_links.len())
            .unwrap_or(u64::MAX)
            .to_le_bytes(),
    );
    for link in evidence_links {
        hasher.update(
            u64::try_from(link.label.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        hasher.update(link.label.as_bytes());
        hasher.update(
            u64::try_from(link.uri.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        hasher.update(link.uri.as_bytes());
        hasher.update(
            u64::try_from(link.content_digest.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
...
fn compute_scoreboard_digest(entries: &BTreeMap<String, ScoreEntry>) -> String {
    ...
    for (claim_id, entry) in entries {
        hasher.update(
            u64::try_from(claim_id.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        hasher.update(claim_id.as_bytes());
        hasher.update(
            u64::try_from(entry.submitter_id.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        hasher.update(entry.submitter_id.as_bytes());
        hasher.update(
            u64::try_from(entry.claim_summary.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        ...
        hasher.update(
            u64::try_from(entry.compilation_digest.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
        hasher.update(entry.compilation_digest.as_bytes());
        hasher.update(
            u64::try_from(entry.evidence_uris.len())
                .unwrap_or(u64::MAX)
                .to_le_bytes(),
        );
