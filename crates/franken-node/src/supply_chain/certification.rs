...
pub fn evaluate_certification(input: &CertificationInput) -> CertificationResult {
    // Delegate to the threshold-aware version with default threshold
    evaluate_certification_with_threshold(input, 80.0)
}
...
/// Internal helper that evaluates certification with a specific test coverage threshold.
#[must_use]
fn evaluate_certification_with_threshold(
    ...
    if attestation_valid {
        ...
        unsatisfied.push("third_party_audit_verified".to_owned());
    }
