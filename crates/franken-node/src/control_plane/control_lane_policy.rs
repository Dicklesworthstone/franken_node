...
impl ControlLanePolicy {
    ...
    pub fn verify_budget_sum(&self) -> bool {
        let total: u16 = self.budgets.values().map(|b| b.min_pct as u16).sum();
        total == 100
    }
    ...
    pub fn tick(
        ...
        let cancel_slots = u32::try_from(total_slots as u64 * CANCEL_LANE_BUDGET_PCT as u64 / 100)
            .unwrap_or(u32::MAX);
        let timed_slots = u32::try_from(total_slots as u64 * TIMED_LANE_BUDGET_PCT as u64 / 100)
            .unwrap_or(u32::MAX);
