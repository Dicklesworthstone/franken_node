...
mod tests {
    ...
    fn reconciliation_resolve_conflict() {
        ...
        if let ReconciliationSuggestion::ResolveConflict { epoch, .. } = suggestion {
            ...
            unreachable!("Expected ResolveConflict");
        }
    }
    ...
    fn time_math_fail_closed_integration_behavior() {
        ...
        assert!(
            far_future > recent_time + (365 * 24 * 3600),
        ...
        assert!(
            far_future > max_reasonable_future,
    ...
    fn time_math_clock_rewind_attack_simulation() {
        ...
        if let Some(pre_epoch) = just_before_epoch {
            ...
            assert_eq!(
                timestamp,
                u64::MAX,
                "Just before UNIX_EPOCH should fail closed"
            );
        }
    ...
    fn time_math_boundary_edge_cases() {
        ...
        assert_eq!(
            future_timestamp,
            u32::MAX as u64,
            "Far future should work normally"
        );
        assert_ne!(
            future_timestamp,
            u64::MAX,
            "Far future should not trigger fallback"
        );
