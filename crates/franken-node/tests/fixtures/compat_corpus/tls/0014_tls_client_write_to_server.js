const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{let b='';sock.on('data',d=>b+=d);sock.on('end',()=>{console.log('server-saw:'+b);sock.end();srv.close();});});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',rejectUnauthorized:false},()=>{c.write('tls-payload');c.end();});
});
