const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{sock.on('end',()=>{console.log('server-end');sock.end();srv.close();});sock.on('data',()=>{});});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',rejectUnauthorized:false},()=>{c.end();});
});
