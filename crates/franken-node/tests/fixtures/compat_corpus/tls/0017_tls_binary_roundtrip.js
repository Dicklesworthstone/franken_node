const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const payload=Buffer.from([0,9,128,255]);
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{sock.on('data',d=>sock.write(d));sock.on('end',()=>sock.end());});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',rejectUnauthorized:false},()=>{c.write(payload);c.end();});
  const chunks=[];c.on('data',d=>chunks.push(d));
  c.on('close',()=>{console.log('hex:'+Buffer.concat(chunks).toString('hex'));srv.close();});
});
