const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY,ALPNProtocols:['h2','http/1.1']},sock=>{sock.end();});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',ALPNProtocols:['http/1.1'],rejectUnauthorized:false},()=>{
    console.log('alpn:'+c.alpnProtocol);c.end();
  });
  c.on('close',()=>srv.close());
});
