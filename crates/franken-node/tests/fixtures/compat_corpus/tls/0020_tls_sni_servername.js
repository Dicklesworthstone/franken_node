const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{console.log('servername:'+sock.servername);sock.end();srv.close();});
srv.listen(0,'127.0.0.1',()=>{
  tls.connect({port:srv.address().port,host:'127.0.0.1',servername:'localhost',rejectUnauthorized:false});
});
