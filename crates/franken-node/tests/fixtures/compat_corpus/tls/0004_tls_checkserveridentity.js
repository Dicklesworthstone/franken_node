const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const ok=tls.checkServerIdentity('localhost',{subject:{CN:'localhost'},subjectaltname:'DNS:localhost'});
const bad=tls.checkServerIdentity('other.example',{subject:{CN:'localhost'},subjectaltname:'DNS:localhost'});
console.log('ok:'+(ok===undefined),'bad:'+(bad instanceof Error));
