...
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{sock.on('data',d=>{sock.end(String(d).toUpperCase());});});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',rejectUnauthorized:false},()=>{c.write('quiet');});
  let b='';c.on('data',d=>b+=d);c.on('end',()=>{console.log('loud:'+b);c.end();srv.close();});
});
