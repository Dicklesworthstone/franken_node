const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{sock.end('hello');});
srv.listen(0,'127.0.0.1',()=>{
  const port=srv.address().port;
  const go=done=>{const c=tls.connect({port,host:'127.0.0.1',rejectUnauthorized:false});let b='';c.on('data',d=>b+=d);c.on('close',()=>done(b));};
  go(a=>{go(b=>{console.log(a+'|'+b);srv.close();});});
});
