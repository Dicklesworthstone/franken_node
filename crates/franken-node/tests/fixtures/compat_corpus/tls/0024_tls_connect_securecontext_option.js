const tls=require('tls');
const CERT=`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
const KEY=`-----BEGIN PRIVATE KEY-----

Dc95hIOE29WCbZoLR99gbNx2
-----END PRIVATE KEY-----`;
const ctx=tls.createSecureContext({});
const srv=tls.createServer({cert:CERT,key:KEY},sock=>{sock.end('ctx-ok');});
srv.listen(0,'127.0.0.1',()=>{
  const c=tls.connect({port:srv.address().port,host:'127.0.0.1',secureContext:ctx,rejectUnauthorized:false});
  let b='';c.on('data',d=>b+=d);c.on('end',()=>{console.log(b);srv.close();});
});
