...
fn remotecap_issue_missing_scope_fails() {
    ...
    let payload: Value = serde_json::from_slice(&output.stdout)
        .expect("issue --json missing --scope must be JSON, not a clap usage dump");
    assert_eq!(
    ...
    assert!(
        ...
        "expected missing-scope denial in JSON, got {payload}"
    );
...
fn remotecap_issue_missing_endpoint_fails() {
    ...
    let output = cmd.assert().failure().get_output().clone();
    let payload: Value = serde_json::from_slice(&output.stdout)
        .expect("issue --json missing --endpoint must be JSON, not a clap usage dump");
    assert_eq!(
        payload["schema_version"].as_str(),
        Some("franken-node/remotecap-error-cli/v1")
    );
    ...
    let error = payload["error"].as_str().unwrap_or_default();
    assert!(
        error.contains("--endpoint must include at least one endpoint prefix"),
...
#[test]
fn remotecap_use_missing_token_file_flag_fails() {
    let workspace = setup_test_workspace();
    let workspace_path = workspace.path().to_str().unwrap();
    ...
    let output = cmd.assert().failure().get_output().clone();
    let payload: Value = serde_json::from_slice(&output.stdout)
        .expect("use --json missing --token-file must be JSON, not a clap usage dump");
    assert_eq!(
        payload["schema_version"].as_str(),
        Some("franken-node/remotecap-error-cli/v1")
    );
...
#[test]
fn remotecap_verify_missing_token_file_flag_fails() {
    let workspace = setup_test_workspace();
    let workspace_path = workspace.path().to_str().unwrap();
    ...
    let output = cmd.assert().failure().get_output().clone();
    let payload: Value = serde_json::from_slice(&output.stdout)
        .expect("verify --json missing --token-file must be JSON, not a clap usage dump");
    assert_eq!(
        payload["schema_version"].as_str(),
        Some("franken-node/remotecap-error-cli/v1")
    );
