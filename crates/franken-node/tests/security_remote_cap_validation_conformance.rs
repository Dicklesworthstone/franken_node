...
fn test_scope_validation_comprehensive() {
    ...
    let scope_vectors: Vec<_> = vectors.into_iter()
        .filter(|v| v.name.starts_with("scope_"))
        .collect();

    for vector in scope_vectors {
        println!("Testing scope vector: {} - {}", vector.name, vector.description);
        ...
        match (&vector.expected_result, &result) {
            ...
            (ExpectedResult::DenyScope { endpoint: expected_endpoint }, Err(RemoteCapError::ScopeDenied { endpoint: actual_endpoint, .. })) => {
                assert_eq!(expected_endpoint, actual_endpoint, "Scope denial endpoint mismatch for vector {}", vector.name);
            },
            (expected, actual) => {
                panic!("Vector {} failed:\nExpected: {:?}\nActual: {:?}", vector.name, expected, actual);
            }
        }
    }
}
...
fn test_expiry_validation_boundaries() {
    let vectors = generate_validation_vectors();
    let expiry_vectors: Vec<_> = vectors.into_iter()
        .filter(|v| v.name.starts_with("expiry_"))
        .collect();

    for vector in expiry_vectors {
        println!("Testing expiry vector: {} - {}", vector.name, vector.description);
...
fn test_replay_protection_comprehensive() {
    ...
    // Test single-use token replay protection
    {
        ...
        assert!(result1.is_ok(), "First use of single-use token should succeed");
        ...
        match result2 {
            ...
            other => panic!("Second use should fail with ReplayDetected, got: {:?}", other),
        }
    }
    ...
    // Test multi-use token (should allow reuse)
    {
        ...
        assert!(result1.is_ok(), "First use of multi-use token should succeed");
        ...
        assert!(result2.is_ok(), "Second use of multi-use token should succeed");
    }
}
...
fn test_audit_logging_comprehensive() {
    ...
    // Test denial audit events
    {
        ...
        assert!(!denial_event.allowed, "Denial event should show not allowed");
        assert!(denial_event.denial_code.is_some(), "Should have denial code");
...
fn test_durable_replay_store_fsync() {
    ...
    assert!(!marker_files.is_empty(), "Should have at least one marker file");

    // Check marker file contains expected content
    let marker_file = marker_files.first().unwrap();
    let marker_content = fs::read_to_string(marker_file.path())
...
fn test_golden_reference_validation() {
    ...
    let success_cases = vec![
        (RemoteOperation::NetworkEgress, "https://api.example.com/data"),
        (RemoteOperation::NetworkEgress, "https://api.example.com/v2/users"),
        (RemoteOperation::FederationSync, "https://api.example.com/federation"),
        (RemoteOperation::FederationSync, "https://control.example.com/admin/sync"),
    ...
    let denial_cases = vec![
        (RemoteOperation::TelemetryExport, "https://api.example.com/data", "operation_not_in_scope"),
        (RemoteOperation::NetworkEgress, "https://malicious.com/data", "endpoint_not_in_scope"),
    ];
    ...
    println!("Golden reference results: {}", serde_json::to_string_pretty(&golden_output).unwrap());

    // Basic validation that our golden reference is complete
    assert!(!golden_results.is_empty(), "Should have golden results");
    assert!(golden_results.keys().any(|k| k.contains("success")), "Should have success cases");
    assert!(golden_results.keys().any(|k| k.contains("denial")), "Should have denial cases");
    assert!(golden_results.keys().any(|k| k.contains("replay")), "Should have replay cases");
}