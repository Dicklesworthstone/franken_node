...
| `franken-node proofs queue status` | Inspect proof queue, proof status, and worker readiness from broker snapshots | `franken-node proofs queue status --input broker-snapshot.json --json` |
| `franken-node proofs workers restart` | Validate and emit proof worker restart requests | `franken-node proofs workers restart --worker-id vmi-proof-1 --operator-id ops-1 --operator-role pipeline_admin --reason outage --confirm --input broker-snapshot.json --json` |
