    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        for seq in test_sequences {
            let proof = match mmr_inclusion_proof(&stream, &cp, seq) {
                Ok(p) => p,
                Err(e) => return TestResult::fail(format!(
            ...
            if let result = comparison::assert_proof_valid(&proof) {
...
impl ConformanceTest for InclusionProofEvictedSequenceTest {
    fn id(&self) -> &str { "R2.2" }
    fn name(&self) -> &str { "Evicted sequence rejection" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn spec_section(&self) -> &str { "2" }
    fn description(&self) -> &str {
...
impl ConformanceTest for InclusionProofOutOfRangeTest {
    fn id(&self) -> &str { "R2.3" }
    fn name(&self) -> &str { "Out-of-range sequence rejection" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn spec_section(&self) -> &str { "2" }
    fn description(&self) -> &str {
    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        match mmr_inclusion_proof(&stream, &cp, 10) {
            ...
            Ok(_) => return TestResult::fail("Should not generate proof for sequence 10 in 5-element stream"),
        }
        ...
        match mmr_inclusion_proof(&stream, &cp, 5) {
            ...
            Ok(_) => return TestResult::fail("Should not generate proof for sequence at stream length"),
        }
...
impl ConformanceTest for InclusionProofStaleCheckpointTest {
    fn id(&self) -> &str { "R2.4" }
    fn name(&self) -> &str { "Stale checkpoint rejection" }
    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        modified_cp.append_marker_hash("different-hash").expect("append");
...
impl ConformanceTest for InclusionProofDisabledTest {
    fn id(&self) -> &str { "R2.5" }
    fn name(&self) -> &str { "Disabled checkpoint rejection" }
    fn category(&self) -> TestCategory { TestCategory::Security }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn spec_section(&self) -> &str { "2" }
    fn description(&self) -> &str {
...
impl ConformanceTest for InclusionProofDeterministicTest {
    fn id(&self) -> &str { "R2.6" }
    fn name(&self) -> &str { "Deterministic proof generation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn spec_section(&self) -> &str { "2" }
    fn description(&self) -> &str {
    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        for seq in test_sequences {
            ...
            let proof1 = mmr_inclusion_proof(&stream, &cp, seq)
                .map_err(|e| TestResult::fail(format!("Failed to generate proof1: {}", e)))?;
            let proof2 = mmr_inclusion_proof(&stream, &cp, seq)
                .map_err(|e| TestResult::fail(format!("Failed to generate proof2: {}", e)))?;
...
impl ConformanceTest for InclusionProofAuditPathLengthTest {
    fn id(&self) -> &str { "R2.7" }
    fn name(&self) -> &str { "Audit path length limit" }
    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        for (tree_size, expected_max_path) in test_cases {
            ...
            let proof = mmr_inclusion_proof(&stream, &cp, last_seq)
                .map_err(|e| TestResult::fail(format!("Failed to generate proof: {}", e)))?;

            if proof.audit_path.len() > expected_max_path {
                return TestResult::fail(format!(
                    "Audit path too long for tree size {}: {} > {} (expected max)",
                    tree_size, proof.audit_path.len(), expected_max_path
                ));
            }
            ...
            if proof.audit_path.len() > computed_bound.min(64) {
                return TestResult::fail(format!(
                    ...
                    tree_size, proof.audit_path.len(), computed_bound
                ));
            }
        }
