...
def generate_policy_golden_artifacts():
    ...
    raise RuntimeError(
        "workspace pressure policy goldens are generated by the Rust library "
        "test using the real policy implementation. Run "
        "UPDATE_GOLDENS=1 rch exec -- env CARGO_TARGET_DIR=/tmp/rch_target_franken_node_pane_6 "
        "cargo test -p frankenengine-node --lib workspace_pressure_policy_decision_golden_matches_real_policy"
    )
...
def load_golden_artifact():
    ...
    if not POLICY_GOLDEN_JSON.exists():
        raise RuntimeError(
            ...
            f"Run the Rust library target with {UPDATE_GOLDENS_ENV}=1 to create it."
        )
    try:
...
def test_golden_artifact_structure():
    ...
    if raw != canonical_json(data):
        print(
            ...
            f"Run the Rust library target with {UPDATE_GOLDENS_ENV}=1."
        )
        return False
