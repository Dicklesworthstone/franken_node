...
fn inv_pra_complete() {
    ...
    assert_eq!(
        store.total_count(),
        6,
        "INV-PRA-COMPLETE: all 6 types must be persistable"
    );
    for t in ArtifactType::all() {
        assert_eq!(
            store.count_by_type(*t),
            1,
            "INV-PRA-COMPLETE: type {} missing",
            t.label()
        );
    }
}
...
fn inv_pra_durable() {
    ...
    assert!(
        store.get("a1").is_some(),
        "INV-PRA-DURABLE: persisted artifact must remain"
    );
    assert!(
...
fn inv_pra_replay() {
    ...
    assert_eq!(
        err.code(),
        "PRA_REPLAY_MISMATCH",
        "INV-PRA-REPLAY: mismatch must be detected"
    );
}
...
fn inv_pra_ordered() {
    ...
    for (i, hook) in hooks.iter().enumerate() {
        assert_eq!(
            hook.replay_order, i as u64,
            "INV-PRA-ORDERED: insertion order preserved"
        );
        assert_eq!(
            hook.sequence_number, i as u64,
            "INV-PRA-ORDERED: sequence monotonic"
        );
    }
}
