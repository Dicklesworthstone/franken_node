"""Unit tests for scripts/check_section_10_2_gate.py (bd-23ys)."""
...
class TestRunAllShape(unittest.TestCase):
    def test_run_all_shape(self) -> None:
        result = mod.run_all()
        self.assertEqual(result["bead_id"], "bd-23ys")
        self.assertEqual(result["section"], "10.2")
    ...
    def test_section_beads_count(self) -> None:
        result = mod.run_all()
        self.assertEqual(len(result["section_beads"]), 12)
...
class TestConstants(unittest.TestCase):
    def test_section_bead_count(self) -> None:
        self.assertEqual(len(mod.SECTION_BEADS), 12)

    def test_domain_group_count(self) -> None:
        self.assertEqual(len(mod.DOMAIN_GROUPS), 12)
