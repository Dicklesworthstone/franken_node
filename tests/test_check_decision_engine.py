"""Tests for scripts/check_decision_engine.py (bd-15u3)."""
...
spec = importlib.util.spec_from_file_location(
    ...
    ROOT / "scripts" / "check_decision_engine.py",
)
...
class TestCheckGuardrailImport(TestCase):
    def test_guardrail(self):
        result = mod.check_guardrail_import()
        self.assertTrue(result["pass"])


class TestCheckBayesianImport(TestCase):
    def test_bayesian(self):
        result = mod.check_bayesian_import()
        self.assertTrue(result["pass"])


class TestCheckPrecedenceLogic(TestCase):
    def test_precedence(self):
        result = mod.check_precedence_logic()
        self.assertTrue(result["pass"])
...
class TestRunChecks(TestCase):
    def test_full_run(self):
        ...
        self.assertEqual(result["bead_id"], "bd-15u3")
...
class TestRequiredConstants(TestCase):
    def test_types_count(self):
        self.assertEqual(len(mod.REQUIRED_TYPES), 5)

    def test_methods_count(self):
        self.assertEqual(len(mod.REQUIRED_METHODS), 3)
    ...
    def test_required_tests_count(self):
        self.assertEqual(len(mod.REQUIRED_TESTS), 34)
...
class TestJsonOutput(TestCase):
    def test_json_serializable(self):
        ...
        self.assertEqual(parsed["bead_id"], "bd-15u3")

    def test_cli_json(self):
        result = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "check_decision_engine.py"), "--json"],
