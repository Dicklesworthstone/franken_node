"""Unit tests for scripts/check_scope_explosion.py (bd-38ri)."""
...
import check_scope_explosion as mod
...
class TestRunAll(unittest.TestCase):
    ...
    def test_structure(self) -> None:
        ...
        self.assertEqual(result["bead_id"], "bd-38ri")
...
class TestIndividualChecks(unittest.TestCase):
    ...
    def test_check_capability_gates(self) -> None:
        mod.check_capability_gates()
        self.assertEqual(len(mod.RESULTS), 1)
        self.assertEqual(mod.RESULTS[0]["name"], "capability_gates")
        self.assertTrue(mod.RESULTS[0]["passed"])

    def test_check_artifact_gates(self) -> None:
        mod.check_artifact_gates()
        self.assertEqual(len(mod.RESULTS), 1)
        self.assertEqual(mod.RESULTS[0]["name"], "artifact_gates")
        self.assertTrue(mod.RESULTS[0]["passed"])

    def test_check_scope_budgets(self) -> None:
        mod.check_scope_budgets()
        self.assertEqual(len(mod.RESULTS), 1)
        self.assertEqual(mod.RESULTS[0]["name"], "scope_budgets")
        self.assertTrue(mod.RESULTS[0]["passed"])
    ...
    def test_check_monitoring(self) -> None:
        mod.check_monitoring()
        self.assertEqual(len(mod.RESULTS), 1)
        self.assertEqual(mod.RESULTS[0]["name"], "monitoring")
        self.assertTrue(mod.RESULTS[0]["passed"])
