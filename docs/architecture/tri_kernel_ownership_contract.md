...
| `crates/franken-node/src/runtime/effect_receipt.rs`, `crates/franken-node/src/storage/cas.rs`, replay/verifier surfaces, and compat oracle gates | `franken_engine` effect producer in `crates/franken-engine/src/hostcall_effects_migration.rs`; first-tranche handoff `bd-f5b04.2.6` | Product-side proof-carrying host-effect receipt, CAS, replay, and release-gate consumer for engine-produced `fs.read` / `fs.write` / `http.request` bytes. | `Wrap canonical ownership` | `franken_node` may not close the first-tranche effect lane by reimplementing `FsHostcallEffect`, `NetworkHostcallEffect`, `FullCapsHandler`, or `dispatches_real_hostcalls` locally. Close requires a pinned `franken_engine` revision where real execution exists and the product consumes its bytes through `EffectReceipt`. | A local producer would let product tests pass while the execution kernel still denies or simulates the same effects, splitting the proof-carrying host-effect truth between repos. | `docs/ENGINE_SPLIT_CONTRACT.md`, `scripts/check_ownership_violations.py`, `tests/test_check_ownership_violations.py`, `crates/franken-node/tests/cas_effect_receipt_conformance.rs` |
...
| `crates/franken-node/src/api/service.rs` | Future request-region/service-boundary ownership | In-process catalog/dispatch layer that assembles route metadata, middleware, and endpoint catalogs, but does not own a real async server boundary. Endpoint reports explicitly mark the boundary as non-live and perf baselines as unavailable pending transport ownership. | `Defer until trigger` | Trigger: a real async HTTP or gRPC boundary with owned request lifecycle, cancellation, and backpressure. Until then, native request-region adoption is premature. | Premature substrate migration would create a fake service runtime and tech debt instead of solving a real ownership problem. | Code-and-doc quarantine for now: `docs/architecture/blueprint.md` plus `crates/franken-node/src/api/service.rs`. Future proof surface lands under `bd-1now.6`. |
...
| `OWN-SEMB-004` | `ENGINE_EFFECT_PRODUCER_BOUNDARY_VIOLATION` | The `bd-f5b04.2.6` effect-producer handoff contract is missing, or `franken_node` defines first-tranche engine hostcall producer symbols locally. | Restore `docs/ENGINE_SPLIT_CONTRACT.md` and route real `fs.read` / `fs.write` / `http.request` production through the pinned `franken_engine` boundary instead of duplicating it locally. |
