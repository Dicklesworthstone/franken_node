...
|------|----------|----------|-----------|-----------------|
| `bd-f5b04.6.1` | Intra-Fleet Secure Aggregation / Cross-Org Differential Privacy | Stage secure aggregation inside one organization's fleet; defer cross-org DP | Secure aggregation and differential privacy solve different problems. Secure aggregation hides which fleet node contributed a signal while preserving the aggregate, making it a low-signal-cost fit for intra-org threat pooling. Differential privacy hides what was contributed by adding noise, which can swamp the rare low-count supply-chain events the ATC, BPET, and DGIS lanes most need to detect. Cross-org DP also depends on a deployment network that does not exist yet, so building it now would add complexity before the project can measure whether the privacy budget is worth the detection loss. | Reconsider cross-org DP only after a real multi-organization deployment exists, rare-event detection remains strong under measured privacy noise, and secure aggregation alone no longer satisfies the sharing threat model. |
| `bd-f5b04.6.2` | Correlated-Campaign Detection | Defer until Phase 0 labels and conformal calibration can feed deterministic campaign-correlation evidence | Coordinated supply-chain campaigns are a real signal, but the build program must not smuggle the rejected Bayesian DBN back in under a lighter name. The acceptable path is a deterministic clustering and correlation pass over trust-card trajectories and the DGIS contagion graph, using calibrated per-package inputs instead of latent-variable inference. Shipping it before the labeled corpus and conformal calibration exist would turn the feature into an unfalsifiable heuristic and risk replay drift from ad hoc scores. | Reconsider after Phase 0 provides labeled campaign examples, conformal coverage is measured for the per-package inputs, and a deterministic graph-correlation design with canonical outputs beats isolated-package scoring on held-out evidence. |
