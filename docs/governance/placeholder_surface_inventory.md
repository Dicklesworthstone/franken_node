...
| `PSI-008` | `truthful_partial_surface` | Ecosystem health export contains placeholder values | `crates/franken-node/src/supply_chain/ecosystem_telemetry.rs::export_health(...)` | Library / export helper | `compromise_reduction_factor` is fixed at `1.0` and `certification_distribution` is empty even when an export is produced. The authoritative formulas and upstream sources are now defined in `docs/specs/section_10_4/bd-phf_contract.md` and the machine-readable helpers `compromise_reduction_factor_contract()` / `certification_distribution_contract()` in `ecosystem_telemetry.rs`, but the live export must still be treated as non-authoritative until `bd-2fqyv.9.2` wires those contracts into real computation and explicit missing-data semantics. | Follow-on remediation bead required under `bd-2fqyv`; scanner policy under `bd-2fqyv.1.2` should flag operator-facing consumers. |
