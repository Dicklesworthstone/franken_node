...
**Follow-up (2026-08-21):** scoreboard bound to corpus 86.43% RED; `run` e2e written; installer/CLI/revocation/10x-equal-attempts honesty landed. 95% corpus still open (`bd-28sz`: 76 remaining). Need 48 more passes (532/560). Do **not** recategorize `child_process` deny as pass. Residual mix: `child_process` 30 engine/policy refuse; `crypto` 13 / `stream` 12 / `zlib` 3 / `tls` 5 / `cluster` 2 mostly franken-engine crash (exit 1); `events::0022` is a 29-vs-30-byte stdout miss (likely missing newline on `EventEmitter.on` this-binding); `net::0005`/`net::0024` are **node vs bun reference disagreement** (franken matches node). Orchestrator cadence locked at **4 minutes**. Spawn note: product already signs Bubblewrap + `echo`/`true` aliases (`compat_corpus_run.rs`); each of the 30 fails has a **distinct** stderr digest (not one shared deny string). On this fleet `/bin/echo` is a rust-coreutils symlink (`/lib/cargo/bin/coreutils/echo`), which `resolve_fixed_path_executable` rejects (uid0 / no-symlink / no group-write). That blocks **local** spawn repro; it is not proof the 2026-08-20 measurement host had the same PATH.
...
| V26 | CLI command surface matches README and does real work | README Command Reference | Important | PARTIAL | **NO open bead for honesty** | No `todo!()`. Thin/misleading: `verify module`, `verify compatibility` auto-PASS, `runtime lane/epoch` ephemeral, `proofs workers restart` paper ticket, `ops health-check` dummy session count, `trust scan --json` ignored. |
