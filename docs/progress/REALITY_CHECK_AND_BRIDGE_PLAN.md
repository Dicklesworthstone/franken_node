...
**Follow-up (2026-08-21):** scoreboard bound to corpus 86.43% RED; `run` e2e written; installer/CLI/revocation/10x-equal-attempts honesty landed. 95% corpus still open (`bd-28sz`: 76 remaining). Need 48 more passes (532/560). Do **not** recategorize `child_process` deny as pass. Residual mix: `child_process` 30 engine/policy refuse; `crypto` 13 / `stream` 12 / `zlib` 3 / `tls` 5 / `cluster` 2 mostly franken-engine crash (exit 1); `events::0022` is a 29-vs-30-byte stdout miss (arrow `this` bound to the emitter would be `fn-this:true` + `arrow-this:true` = 29 bytes vs node/bun 30); `net::0005`/`net::0024` are **node vs bun reference disagreement** (franken matches node). Orchestrator cadence locked at **4 minutes**. Spawn note: signed Bubblewrap + aliases were already in the 2026-08-20 measurement (`e22e89fc4` is an ancestor of artifact commit `cbc7a7bfb`). Each of the 30 fails has a **distinct** 229-byte stderr digest (not one shared deny string). On this fleet `/usr/bin/echo` is a rust-coreutils symlink whose **canonical** target `/usr/lib/cargo/bin/coreutils/echo` is uid0 mode 755 with uid0 ancestors — PATH resolution is not the measured 30-fail cause. `resolve_fixed_path_executable` now **skips** an insecure first PATH hit instead of bailing, then still fail-closes if no uid0 match remains.
