...
IMPL_PATH = ROOT / "crates/franken-node/src/connector/execution_scorer.rs"
FIXTURES_PATH = ROOT / "artifacts/section_10_13/bd-jxgt/planner_decision_explanations.json"
INTEGRATION_PATH = ROOT / "tests/integration/execution_planner_determinism.rs"
SPEC_PATH = ROOT / "docs/specs/section_10_13/bd-jxgt_contract.md"
EVIDENCE_PATH = ROOT / "artifacts/section_10_13/bd-jxgt/verification_evidence.json"
...
def run_rust_tests() -> tuple[bool, str]:
    try:
        result = subprocess.run(
            ["rch", "exec", "--", "cargo", "test", "-p", "frankenengine-node", "--", "connector::execution_scorer"],
            capture_output=True,
            text=True,
            timeout=3600,
...
def run_checks(*, run_tests: bool, emit_human: bool) -> dict[str, object]:
    ...
    if content is not None:
        ...
        all_types = False
    check(checks, "EPS-IMPL", "Implementation with all required types", impl_exists and all_types, emit_human=emit_human)
    ...
    if content is not None:
        ...
        check(checks, "EPS-ERRORS", "Error codes", False, emit_human=emit_human)

    fixtures_valid = False
    fixture_content = read_utf8(FIXTURES_PATH)
    if fixture_content is not None:
        try:
            ...
            pass
    check(checks, "EPS-FIXTURES", "Planner decision explanation fixtures", fixtures_valid, emit_human=emit_human)
    ...
    if integration_content is not None:
        has_det = "inv_eps_deterministic" in integration_content
        has_tie = "inv_eps_tiebreak" in integration_content
        has_exp = "inv_eps_explainable" in integration_content
        has_rej = "inv_eps_reject_invalid" in integration_content
    else:
    ...
    if spec_content is not None:
        has_invariants = "INV-EPS" in spec_content
        has_types = "ExecutionScorer" in spec_content and "ScoringWeights" in spec_content
    else:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-evidence",
        action="store_true",
        help="write artifacts/section_10_13/bd-jxgt/verification_evidence.json; human mode writes by default",
    )
