...
IMPL_PATH = ROOT / "crates/franken-node/src/connector/repair_controller.rs"
CSV_PATH = ROOT / "artifacts/section_10_13/bd-91gg/repair_cycle_telemetry.csv"
INTEGRATION_PATH = ROOT / "tests/integration/repair_fairness.rs"
SPEC_PATH = ROOT / "docs/specs/section_10_13/bd-91gg_contract.md"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-evidence",
        action="store_true",
        help="write artifacts/section_10_13/bd-91gg/verification_evidence.json; human mode writes by default",
    )
...
def run_checks(*, run_tests: bool, emit_human: bool) -> dict[str, object]:
    ...
    if content is not None:
        ...
        check(checks, "BRC-ERRORS", "Error codes", False, emit_human=emit_human)
    ...
    if csv_content is not None:
        ...
        csv_details = f"found {len(lines)} non-empty rows"
    else:
        csv_details = f"unable to read {CSV_PATH}"
    check(
        checks,
        "BRC-TELEMETRY",
        "Repair cycle telemetry CSV",
        csv_valid,
        csv_details,
        emit_human=emit_human,
    )
    ...
    if integration_content is not None:
        has_bounded = "inv_brc_bounded" in integration_content
        has_fairness = "inv_brc_fairness" in integration_content
        has_audit = "inv_brc_auditable" in integration_content
        has_det = "inv_brc_deterministic" in integration_content
    else:
    ...
    if spec_content is not None:
        has_invariants = "INV-BRC" in spec_content
        has_types = "BackgroundRepairController" in spec_content and "RepairConfig" in spec_content
    else:
