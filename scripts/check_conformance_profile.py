...
IMPL_PATH = ROOT / "crates/franken-node/src/connector/conformance_profile.rs"
REPORT_PATH = ROOT / "artifacts/section_10_13/bd-ck2h/profile_claim_report.json"
INTEGRATION_PATH = ROOT / "tests/integration/profile_claim_gate.rs"
SPEC_PATH = ROOT / "docs/specs/section_10_13/bd-ck2h_contract.md"
EVIDENCE_PATH = ROOT / "artifacts/section_10_13/bd-ck2h/verification_evidence.json"
...
def run_rust_tests() -> tuple[bool, str]:
    try:
        result = subprocess.run(
            ["rch", "exec", "--", "cargo", "test", "-p", "frankenengine-node", "--", "connector::conformance_profile"],
            capture_output=True,
            text=True,
            timeout=3600,
...
def run_checks(*, run_tests: bool, emit_human: bool) -> dict[str, object]:
    ...
    if content is not None:
        ...
        all_types = False
    check(checks, "CPM-IMPL", "Implementation with all required types", impl_exists and all_types, emit_human=emit_human)
    ...
    if content is not None:
        ...
        check(checks, "CPM-ERRORS", "Error codes", False, emit_human=emit_human)
    ...
    if report_content is not None:
        try:
            ...
            pass
    check(checks, "CPM-REPORT", "Profile claim report fixture", report_valid, emit_human=emit_human)
    ...
    if integration_content is not None:
        has_matrix = "inv_cpm_matrix" in integration_content
        has_measured = "inv_cpm_measured" in integration_content
        has_blocked = "inv_cpm_blocked" in integration_content
        has_metadata = "inv_cpm_metadata" in integration_content
    else:
    ...
    if spec_content is not None:
        has_invariants = "INV-CPM" in spec_content
        has_types = "ProfileMatrix" in spec_content and "ClaimEvaluation" in spec_content
    else:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-evidence",
        action="store_true",
        help="write artifacts/section_10_13/bd-ck2h/verification_evidence.json; human mode writes by default",
    )
