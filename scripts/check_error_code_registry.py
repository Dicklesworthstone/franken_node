...
IMPL_PATH = ROOT / "crates/franken-node/src/connector/error_code_registry.rs"
CATALOG_PATH = ROOT / "artifacts/section_10_13/bd-novi/error_code_registry.json"
INTEGRATION_PATH = ROOT / "tests/integration/error_contract_stability.rs"
SPEC_PATH = ROOT / "docs/specs/section_10_13/bd-novi_contract.md"
EVIDENCE_PATH = ROOT / "artifacts/section_10_13/bd-novi/verification_evidence.json"
...
def run_rust_tests() -> tuple[bool, str]:
    try:
        result = subprocess.run(
            ["rch", "exec", "--", "cargo", "test", "-p", "frankenengine-node", "--", "connector::error_code_registry"],
            capture_output=True,
            text=True,
            timeout=3600,
...
def run_checks(*, run_tests: bool, emit_human: bool) -> dict[str, object]:
    ...
    if content is not None:
        ...
        all_types = False
    check(checks, "ECR-IMPL", "Implementation with all required types", impl_exists and all_types, emit_human=emit_human)
    ...
    if content is not None:
        ...
        check(checks, "ECR-ERRORS", "Error codes", False, emit_human=emit_human)

    catalog_valid = False
    catalog_content = read_utf8(CATALOG_PATH)
    if catalog_content is not None:
        try:
            data = json.loads(catalog_content)
            catalog_valid = "error_codes" in data and len(data["error_codes"]) >= 4
        except json.JSONDecodeError:
            pass
    check(checks, "ECR-CATALOG", "Error code registry fixture", catalog_valid, emit_human=emit_human)
    ...
    if integration_content is not None:
        has_namespaced = "inv_ecr_namespaced" in integration_content
        has_unique = "inv_ecr_unique" in integration_content
        has_recovery = "inv_ecr_recovery" in integration_content
        has_frozen = "inv_ecr_frozen" in integration_content
    else:
    ...
    if spec_content is not None:
        has_invariants = "INV-ECR" in spec_content
        has_types = "ErrorCodeRegistry" in spec_content and "RecoveryInfo" in spec_content
    else:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-evidence",
        action="store_true",
        help="write artifacts/section_10_13/bd-novi/verification_evidence.json; human mode writes by default",
    )
