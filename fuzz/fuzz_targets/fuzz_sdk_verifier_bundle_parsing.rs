...
fn fuzz_bundle_structured(input: FuzzReplayBundle) {
    ...
    if let Ok(json) = serde_json::to_vec(&bundle) {
        ...
        assert_eq!(
            hash_result.len(),
            64,
            "Hash function should produce a SHA-256 digest"
        );

        // Verify should handle the JSON safely
        let _verify_result = verify(&json);
    ...
    if let Ok(pretty_json) = serde_json::to_vec_pretty(&bundle) {
        ...
        let _pretty_verify = verify(&pretty_json);

        // Pretty-printed JSON should parse consistently
        if let Ok(json) = serde_json::to_vec(&bundle) {
            let normal_deserialize = deserialize(&json);
            assert_eq!(
                pretty_deserialize.is_ok(),
                normal_deserialize.is_ok(),
    ...
    if let Ok(digest) = integrity_result {
        assert_eq!(
            digest.len(),
            64,
            "Integrity hash should produce a SHA-256 digest"
        );
    }
...
fn fuzz_timeline_event_structured(input: FuzzTimelineEvent) {
    ...
    if let Ok(json) = serde_json::to_vec(&event) {
        ...
        assert!(
            parsed_event.is_ok(),
            "Timeline event round-trip should succeed"
        );
    }
...
fn fuzz_bundle_header_structured(header: BundleHeader) {
    if let Ok(json) = serde_json::to_vec(&header) {
        ...
        assert!(
            parsed_header.is_ok(),
            "BundleHeader round-trip should succeed"
        );
    }
...
fn fuzz_bundle_chunk_structured(chunk: BundleChunk) {
    if let Ok(json) = serde_json::to_vec(&chunk) {
        ...
        assert!(
            parsed_chunk.is_ok(),
            "BundleChunk round-trip should succeed"
        );
    }
...
fn fuzz_bundle_artifact_structured(artifact: BundleArtifact) {
    if let Ok(json) = serde_json::to_vec(&artifact) {
        ...
        assert!(
            parsed_artifact.is_ok(),
            "BundleArtifact round-trip should succeed"
        );
    }
...
fn fuzz_bundle_raw_bytes(bytes: Vec<u8>) {
    ...
    assert_eq!(
        deserialize_result1.is_ok(),
        deserialize_result2.is_ok(),
    ...
    if bytes.len() < 10 && !bytes.is_empty() {
        assert!(
            ...
            "Very small input should not parse as ReplayBundle"
        );
        assert!(
            header_result.is_err(),
            "Very small input should not parse as BundleHeader"
        );
    }
    ...
    if bytes.is_empty() {
        assert!(
            bundle_result.is_err(),
            "Empty input should not parse as ReplayBundle"
        );
        assert!(
            deserialize_result1.is_err(),
            "Empty input should not deserialize via SDK"
        );
    }
    ...
    if let Ok(bundle) = deserialize(&bytes) {
        ...
        assert_eq!(
            integrity_result1.is_ok(),
            integrity_result2.is_ok(),
        ...
        assert!(
            ...
            "Verify should fail if deserialization fails"
        );
    }
}
...
fn fuzz_hex_parse(candidate: &str) {
    ...
    if let (Ok(bytes1), Ok(bytes2)) = (hex_result1, hex_result2) {
        assert_eq!(
            bytes1, bytes2,
            "Hex parsing should produce identical results"
        );
    }
    ...
    if candidate.is_empty() {
        ...
        assert!(
            empty_result.is_ok(),
            "Empty string should parse as empty hex"
        );
    }
    ...
    if candidate.len() % 2 == 1 && !candidate.is_empty() {
        ...
        assert!(
            odd_result.is_err(),
            "Odd-length hex string should be rejected"
        );
    }
    ...
    if candidate.len() % 2 == 0 && candidate.chars().all(|c| c.is_ascii_hexdigit()) {
        ...
        assert!(
            valid_result.is_ok(),
            "Valid even-length hex should parse successfully"
        );
    }
}
