...
use frankenengine_node::migration::rewrite_suggestion_engine::{
    generate_suggestions, generate_suggestions_from_scan, generate_rollback_plan_at,
    generate_report, RewriteApiUsage, RewriteRiskLevel, RewriteCategory, RewriteSuggestion,
    RewriteSuggestionReport, RewriteRollbackPlan,
};
use frankenengine_node::supply_chain::project_scanner::{
    ProjectScanReport, ApiUsage, RiskLevel, DependencyInfo, PackageInfo,
};
...
struct FuzzDependencyInfo {
    ...
    name: String,
    #[arbitrary(with = bounded_version)]
    version: String,
    #[arbitrary(with = bounded_source)]
    source: String,
    risk_level: RiskLevel,
}

impl From<FuzzDependencyInfo> for DependencyInfo {
...
/// Fuzzable package info
#[derive(Debug, Clone, Arbitrary)]
struct FuzzPackageInfo {
    #[arbitrary(with = bounded_package_name)]
    name: String,
    #[arbitrary(with = bounded_version)]
    version: String,
...
impl From<FuzzPackageInfo> for PackageInfo {
    fn from(fuzz: FuzzPackageInfo) -> Self {
        Self {
            name: fuzz.name,
            version: fuzz.version,
...
struct FuzzProjectScanReport {
    ...
    project_name: String,
    #[arbitrary(with = bounded_source_dir)]
    ...
    package_info: FuzzPackageInfo,
}

impl From<FuzzProjectScanReport> for ProjectScanReport {
    fn from(fuzz: FuzzProjectScanReport) -> Self {
        Self {
            project_name: fuzz.project_name,
            source_directory: fuzz.source_directory,
            api_usage: fuzz.api_usage.into_iter().map(|u| u.into()).collect(),
            dependencies: fuzz.dependencies.into_iter().map(|d| d.into()).collect(),
            package_info: fuzz.package_info.into(),
        }
    }
}
...
fn bounded_source(u: &mut Unstructured) -> arbitrary::Result<String> {
...
fn bounded_description(u: &mut Unstructured) -> arbitrary::Result<Option<String>> {
    if u.arbitrary::<bool>()? {
...
fn bounded_source_dir(u: &mut Unstructured) -> arbitrary::Result<String> {
    bounded_source_file(u) // Same logic
}
...
fuzz_target!(|data: &[u8]| {
    ...
    for op in input.operations {
        match op {
            RewriteOperation::GenerateSuggestionsFromUsages { usages } => {
                let rewrite_usages: Vec<RewriteApiUsage> = usages.into_iter().map(|u| u.into()).collect();
                ...
                for suggestion in &suggestions {
                    ...
                    assert!(
                        matches!(suggestion.risk_level,
                                RewriteRiskLevel::Critical | RewriteRiskLevel::High |
                                RewriteRiskLevel::Medium | RewriteRiskLevel::Low),
                        "Risk level should be valid enum variant"
                    );

                    // Categories should be valid
                    assert!(
                        matches!(suggestion.category,
                                RewriteCategory::DirectReplacement | RewriteCategory::AdapterNeeded |
                                RewriteCategory::RemovalNeeded | RewriteCategory::ManualReview),
                    ...
                    if let Some(line_num) = suggestion.line_number {
                        ...
                        assert!(line_num <= MAX_LINE_NUMBER, "Line numbers should be reasonable");
                    }

                    // Descriptions should not be empty for valid suggestions
                    if !suggestion.api_family.is_empty() && !suggestion.api_name.is_empty() {
                        assert!(!suggestion.description.is_empty(), "Valid suggestions should have descriptions");
                    }

                    // Before/after code should be different for actual rewrites
                    if suggestion.category == RewriteCategory::DirectReplacement &&
                       !suggestion.before.is_empty() && !suggestion.after.is_empty() {
                        assert_ne!(suggestion.before, suggestion.after,
                ...
                for window in suggestions.windows(2) {
                    let left_priority = window[0].risk_level.priority();
                    let right_priority = window[1].risk_level.priority();
            ...
            RewriteOperation::GenerateSuggestionsFromScan { scan_report } => {
                ...
                assert!(suggestions.len() <= project_scan.api_usage.len(),
                ...
                for suggestion in &suggestions {
                    ...
                    if !suggestion.api_family.is_empty() && !suggestion.api_name.is_empty() {
                        assert!(matches_usage, "Suggestion should correspond to actual API usage");
                    }
                }
            }

            RewriteOperation::GenerateRollbackPlan { usages, project, timestamp } => {
                let rewrite_usages: Vec<RewriteApiUsage> = usages.into_iter().map(|u| u.into()).collect();
                let suggestions = generate_suggestions(&rewrite_usages);
                ...
                assert_eq!(rollback_plan.generated_at, timestamp, "Timestamp should match");
                assert_eq!(rollback_plan.suggestion_count as usize, suggestions.len(),
                ...
                assert_eq!(category_sum, rollback_plan.suggestion_count,
                ...
                assert_eq!(unique_files.len(), rollback_plan.affected_files.len(),
            ...
            RewriteOperation::GenerateFullReport { scan_report } => {
                let project_scan: ProjectScanReport = scan_report.into();
                let report = generate_report(&project_scan);
                report_count += 1;

                // Verify report structure
                assert_eq!(report.schema_version, "franken_node/migration/rewrite_suggestion_report/v1",
                          "Schema version should be correct");
                assert_eq!(report.project, project_scan.project_name, "Project name should match");

                // Report should include all components
                assert_eq!(report.suggestions.len(), report.rollback_plan.suggestion_count as usize,
                          "Suggestion counts should match between report and rollback plan");

                // Summary should be consistent
                assert!(report.summary.total_suggestions >= report.summary.critical_suggestions,
                       "Total suggestions should be >= critical suggestions");
                assert!(report.summary.total_suggestions >= report.summary.high_suggestions,
                       "Total suggestions should be >= high suggestions");

                // Category breakdown should sum to total
                let category_total = report.summary.category_breakdown.values().sum::<u64>();
                assert_eq!(category_total, report.summary.total_suggestions,
                          "Category breakdown should sum to total");
            }
            ...
            RewriteOperation::TestSerialization { usages, project } => {
                ...
                let rewrite_usages: Vec<RewriteApiUsage> = usages.into_iter().map(|u| u.into()).collect();
                let suggestions = generate_suggestions(&rewrite_usages);
                let rollback_plan = generate_rollback_plan_at(&suggestions, &project, "2026-05-22T16:10:00Z");
                ...
                let report = generate_report(&scan_report);
                ...
                match serde_json::to_string(&report) {
                    Ok(json_str) => {
                        ...
                        assert!(json_str.contains("suggestions"), "JSON should contain suggestions");
                        assert!(json_str.contains("rollback_plan"), "JSON should contain rollback plan");
                        ...
                        match serde_json::from_str::<RewriteSuggestionReport>(&json_str) {
                            Ok(deserialized) => {
                                ...
                                assert_eq!(report.project, deserialized.project,
                                         "Project should survive round-trip");
                                assert_eq!(report.suggestions.len(), deserialized.suggestions.len(),
            ...
            RewriteOperation::TestRiskLevelConversion { risk_levels } => {
                ...
                for risk_level in risk_levels {
                    let rewrite_risk: RewriteRiskLevel = risk_level.into();
                    let priority = rewrite_risk.priority();
                    ...
                    assert_eq!(risk_level, back_to_original, "Risk level conversion should be consistent");
                }
            }
                ...
                for suggestion in &suggestions {
                    ...
                    match suggestion.api_family.as_str() {
                        "fs" | "crypto" | "path" => {
                            ...
                            assert!(matches!(suggestion.category,
    ...
    assert!(suggestion_count >= 0, "Suggestion count should be non-negative");
    assert!(rollback_plan_count >= 0, "Rollback plan count should be non-negative");
    assert!(report_count >= 0, "Report count should be non-negative");
    assert!(successful_serializations <= serialization_attempts,
    ...
    assert!(empty_suggestions.is_empty(), "Empty usages should produce empty suggestions");

    let empty_rollback = generate_rollback_plan_at(&empty_suggestions, "test", "2026-01-01T00:00:00Z");
    assert_eq!(empty_rollback.suggestion_count, 0, "Empty suggestions should produce empty rollback plan");
    assert!(empty_rollback.affected_files.is_empty(), "Empty rollback should have no affected files");
    assert!(empty_rollback.rollback_commands.is_empty(), "Empty rollback should have no commands");
    ...
    assert!(!long_suggestions.is_empty(), "Long strings should still produce suggestions");

    // Verify the suggestion properties are preserved
    if let Some(suggestion) = long_suggestions.first() {
        assert!(suggestion.api_family.len() <= 1000, "API family length should be bounded");
        assert!(suggestion.api_name.len() <= 1000, "API name length should be bounded");
        assert!(suggestion.source_file.len() <= 1000, "Source file length should be bounded");
    }
});