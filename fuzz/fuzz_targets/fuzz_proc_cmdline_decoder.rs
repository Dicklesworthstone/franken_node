...
impl SeparatorVariant {
    fn apply(&self, args: &[String]) -> Vec<u8> {
        match self {
            SeparatorVariant::Standard => args
                .iter()
                .flat_map(|arg| [arg.as_bytes(), &[0]].concat())
                .collect(),
            SeparatorVariant::Multiple => args
                .iter()
            ...
            SeparatorVariant::Embedded => args
                .iter()
                .flat_map(|arg| {
                    let mid = arg.len() / 2;
...
impl NullByteAttackType {
    fn generate(&self, base_args: &[String]) -> Vec<u8> {
        match self {
            ...
            NullByteAttackType::NullInMiddle => base_args
                .iter()
                .flat_map(|arg| {
                    let mid = arg.len() / 2;
                    [
...
impl UnicodeAttackType {
    fn inject(&self, payload: &str) -> Vec<u8> {
        let attack_data = match self {
            ...
            UnicodeAttackType::ZeroWidth => format!("arg\u{200B}\u{FEFF}{}", payload)
                .as_bytes()
                .to_vec(),
            UnicodeAttackType::Normalization => "café".as_bytes().to_vec(), // NFC vs NFD
            UnicodeAttackType::BidiOverride => format!("\u{202D}arg{}\u{202C}", payload)
                .as_bytes()
                .to_vec(),
...
impl EdgeCaseType {
    fn generate(&self, repetition: u8) -> Vec<u8> {
        ...
        match self {
            ...
            EdgeCaseType::AlternatingBytes => (0..size)
                .map(|i| if i % 2 == 0 { 0xAA } else { 0x55 })
                .collect(),
            EdgeCaseType::IncrementingBytes => (0..size).map(|i| (i % 256) as u8).collect(),
            EdgeCaseType::DecrementingBytes => (0..size).map(|i| (255 - (i % 256)) as u8).collect(),
            EdgeCaseType::RandomPattern => (0..size).map(|i| ((i * 17 + 13) % 256) as u8).collect(),
            EdgeCaseType::SpecialValues => {
                let special = [0x00, 0x01, 0x20, 0x7F, 0x80, 0xFE, 0xFF];
                special.iter().cycle().take(size).copied().collect()
            }
            EdgeCaseType::RepeatedPattern => b"cmd\0".iter().cycle().take(size).copied().collect(),
        }
    }
}
...
impl EncodingType {
    fn apply(&self, base_data: &[u8]) -> Vec<u8> {
        match self {
            ...
            EncodingType::InvalidUtf8 => [base_data, &[0xC0, 0x80, 0xFE, 0xFF, 0]].concat(),
...
impl FormatConfusionType {
    fn apply(&self, modifier: u8) -> Vec<u8> {
        match self {
            FormatConfusionType::SpaceDelimited => b"prog arg1 arg2 arg3".to_vec(),
            FormatConfusionType::TabDelimited => b"prog\targ1\targ2\targ3".to_vec(),
            FormatConfusionType::NewlineDelimited => b"prog\narg1\narg2\narg3".to_vec(),
            FormatConfusionType::JsonLike => br#"{"cmd":"prog","args":["arg1","arg2"]}"#.to_vec(),
            FormatConfusionType::QuotedArgs => {
                b"prog \"arg with spaces\" 'single quoted' arg".to_vec()
            }
            FormatConfusionType::EscapedChars => b"prog\\x20arg\\n\\t\\r".to_vec(),
            FormatConfusionType::MixedDelimiters => {
                let delim: &[u8] = match modifier % 4 {
                    0 => &b"\0"[..],
                    1 => &b" "[..],
                    2 => &b"\t"[..],
                    _ => &b"\n"[..],
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(fuzz_input) = FuzzInput::arbitrary(&mut u) {
        match fuzz_input.operation {
            ProcCmdlineDecodeTest::ValidCmdlines {
                args,
                separator_variant,
            } => {
                ...
                assert_eq!(
                    result1.len(),
                    result2.len(),
                    "Valid cmdline parsing should be deterministic"
                );
            }
            ProcCmdlineDecodeTest::NullByteAttacks {
                attack_type,
                base_args,
            } => {
            ...
            ProcCmdlineDecodeTest::BoundaryConditions {
                boundary_type,
                size,
            } => {
            ...
            ProcCmdlineDecodeTest::UnicodeAttacks {
                unicode_type,
                payload,
            } => {
            ...
            ProcCmdlineDecodeTest::InjectionAttacks {
                injection_type,
                target_arg,
            } => {
            ...
            ProcCmdlineDecodeTest::EdgeCases {
                edge_type,
                repetition,
            } => {
            ...
            ProcCmdlineDecodeTest::EncodingTests {
                encoding_type,
                base_data,
            } => {
            ...
            ProcCmdlineDecodeTest::FormatConfusion {
                confusion_type,
                modifier,
            } => {
