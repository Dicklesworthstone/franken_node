...
fuzz_target!(|data: &[u8]| {
    ...
    if str::from_utf8(data).is_err() {
        // Invalid UTF-8 must be rejected before it reaches the RFC3339 parser.
        return;
    }
    ...
    if let Ok(rfc3339_str) = str::from_utf8(data) {
        ...
        if let Ok(timestamp) = QuarantineAuditTimestamp::try_from(rfc3339_str) {
            ...
            assert!(
                reparsed.is_ok(),
                "canonical format should parse back successfully"
            );
            ...
            assert_eq!(
                canonical, displayed,
                "Display should match canonical format"
            );
            ...
            assert_eq!(
                timestamp, converted_back,
                "DateTime conversion should be reversible"
            );
            ...
            assert!(
                !canonical.is_empty(),
                "canonical format should not be empty"
            );
            assert!(
                canonical.contains('T'),
                "canonical format should contain T separator"
            );
            assert!(
                canonical.ends_with('Z'),
                "canonical format should end with Z"
            );
        }
