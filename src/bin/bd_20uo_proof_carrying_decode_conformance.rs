//! bd-20uo Proof-Carrying Decode Conformance Test Runner
//!
//! This binary executes the complete bd-20uo conformance test suite and generates
...
mod bd_20uo_proof_carrying_decode_conformance {
    include!("../../tests/conformance/bd_20uo_proof_carrying_decode_conformance.rs");
}

fn main() {
    println!("🚀 bd-20uo Proof-Carrying Decode Conformance Test Suite");
    println!("=======================================================\n");

    // Run the conformance test suite
    let report = bd_20uo_proof_carrying_decode_conformance::run_bd20uo_conformance_tests();
    ...
    if compliance_score >= 0.95 {
        println!("✅ CONFORMANT - Meets bd-20uo specification requirements");
    } else {
        println!("❌ NON-CONFORMANT - Does not meet bd-20uo specification requirements");

        // List failed MUST requirements
        println!("\n🔍 Failed MUST Requirements:");
        for record in report.results.values() {
            if let (
                bd_20uo_proof_carrying_decode_conformance::RequirementLevel::Must,
                bd_20uo_proof_carrying_decode_conformance::TestResult::Fail { reason }
    ...
    std::fs::write("bd_20uo_conformance_report.json", json_report)
        .expect("Failed to write JSON report");
    println!("  📄 JSON: bd_20uo_conformance_report.json");

    // Markdown report for human consumption
    let markdown_report = report.to_markdown();
    std::fs::write("bd_20uo_conformance_report.md", markdown_report)
        .expect("Failed to write Markdown report");
    println!("  📄 Markdown: bd_20uo_conformance_report.md");
