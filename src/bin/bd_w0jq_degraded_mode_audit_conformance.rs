//! bd-w0jq Degraded-mode Audit Conformance Test Runner
//!
//! This binary executes the complete bd-w0jq conformance test suite and generates
...
mod bd_w0jq_degraded_mode_audit_conformance {
    include!("../../tests/conformance/bd_w0jq_degraded_mode_audit_conformance.rs");
}

fn main() {
    println!("🚀 bd-w0jq Degraded-mode Audit Conformance Test Suite");
    println!("====================================================\n");

    // Run the conformance test suite
    let report = bd_w0jq_degraded_mode_audit_conformance::run_bd_w0jq_conformance_tests();
    ...
    if compliance_score >= 0.95 {
        println!("✅ CONFORMANT - Meets bd-w0jq specification requirements");
    } else {
        println!("❌ NON-CONFORMANT - Does not meet bd-w0jq specification requirements");

        // List failed MUST requirements
        println!("\n🔍 Failed MUST Requirements:");
        for record in report.results.values() {
            if let (
                bd_w0jq_degraded_mode_audit_conformance::RequirementLevel::Must,
                bd_w0jq_degraded_mode_audit_conformance::TestResult::Fail { reason }
    ...
    std::fs::write("bd_w0jq_conformance_report.json", json_report)
        .expect("Failed to write JSON report");
    println!("  📄 JSON: bd_w0jq_conformance_report.json");

    // Markdown report for human consumption
    let markdown_report = report.to_markdown();
    std::fs::write("bd_w0jq_conformance_report.md", markdown_report)
        .expect("Failed to write Markdown report");
    println!("  📄 Markdown: bd_w0jq_conformance_report.md");
